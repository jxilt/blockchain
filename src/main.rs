...
pub fn main() {
    ...
    let address = format!("0.0.0.0:{}", port.expect("TODO: Remove"));
...
fn extract_port_from_args(mut args: Args) -> Result<String, String> {
    ...
    let maybe_port = args.next();
    return match maybe_port {
        None => Err("Flag \"-p\" used but no port provided.".to_string()),
        Some(port) => {
            let maybe_numeric_port = port.parse::<i32>();

            match maybe_numeric_port {
                Ok(_) => {
                    println!("Using provided port of {}.", port);
                    Ok(port)
                },
                Err(_) => Err(format!("Flag \"-p\" used but port had incorrect format: {}.", port))
            }
        }
    }
}
...
fn loop_until_exit_requested<R: BufRead>(mut reader: R) -> Result<(), Error> {
